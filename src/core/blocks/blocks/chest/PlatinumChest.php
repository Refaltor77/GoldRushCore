...
class PlatinumChest extends Chest implements Permutable
{
    use RotatableTrait;
    ...
    public function onInteract(Item $item, int $face, Vector3 $clickVector, ?Player $player = null, array &$returnedItems = []) : bool{
        ...
        if($player instanceof Player){
            ...
            if($chest instanceof PlatinumChestTile){
                if ($chest->isLocked) {
                    $code = $chest->getCode();
                        ...
                        $keypadInv->setAcceptCallback(function (string $codeInput) use ($player, $pos): void {
                            if ($player->isConnected()) {
                                ...
                                if ($chest instanceof PlatinumChestTile) {
                                    ...
                                    $chest->setLocked($codeInput);
                                    $pos->getWorld()->setBlock($pos, CustomiesBlockFactory::getInstance()->get(BlockIds::PLATINUM_CHEST_LOCK));
                                    $player->setCurrentWindow($chest->getInventory());
                                    $player->sendSuccessSound();
                                } else {
                                    $player->removeCurrentWindow();
