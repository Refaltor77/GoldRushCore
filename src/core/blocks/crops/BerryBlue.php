...
class BerryBlue extends CustomCropsNoDirtFertil
{
    ...
    public function onInteract2(Item $item, int $face, Vector3 $clickVector, ?Player $player = null, array &$returnedItems = []) : bool{
        ...
        if($data->getAge() < 3 && $item instanceof Fertilizer && $player instanceof CustomPlayer){
            ...
            switch ($data->getAge()) {
                case 0:
                    $block = CustomiesBlockFactory::getInstance()->get(Ids::BERRY_BLUE_CROPS_STAGE_1);
                    $data = Main::getInstance()->blockDataService->get($this->getPosition()->getWorld())->getBlockDataAt(
                        $this->getPosition()->getX(),
                        $this->getPosition()->getY(),
                        $this->getPosition()->getZ()
                    ...
                    break;
                case 1:
                    $block = CustomiesBlockFactory::getInstance()->get(Ids::BERRY_BLUE_CROPS_STAGE_2);
                    $data = Main::getInstance()->blockDataService->get($this->getPosition()->getWorld())->getBlockDataAt(
                        $this->getPosition()->getX(),
                        $this->getPosition()->getY(),
                        $this->getPosition()->getZ()
                    ...
                    break;
                case 2:
                    $block = CustomiesBlockFactory::getInstance()->get(Ids::BERRY_BLUE_CROPS_STAGE_3);
                    $data = Main::getInstance()->blockDataService->get($this->getPosition()->getWorld())->getBlockDataAt(
                        $this->getPosition()->getX(),
                        $this->getPosition()->getY(),
                        $this->getPosition()->getZ()
    ...
    public function getDrops(Item $item): array
    {
        if ($this->ageCustom >= 3) {
            return [
                CustomiesItemFactory::getInstance()->get(Ids::BERRY_BLUE, 2)
            ];
        } else {
            return [
                CustomiesItemFactory::getInstance()->get(Ids::BERRY_BLUE)
            ];
        }
    }
    ...
    public function onInteract(Item $item, int $face, Vector3 $clickVector, ?Player $player = null, array &$returnedItems = []) : bool{
        ...
        if ($data instanceof BlockHistoryData) {
            if($data->getAge() >= 3){
                ...
                $world->dropItem($this->position, CustomiesItemFactory::getInstance()->get(Ids::BERRY_BLUE));
