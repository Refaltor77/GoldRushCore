...
class RaisinCrops extends CustomCropsNoDirtFertil
{
    ...
    public function onRandomTick(): void
    {
        ...
        if($data->getAge() < 3 && mt_rand(0, 2) === 1){
            switch ($data->getAge()) {
                case 0:
                    $block = CustomiesBlockFactory::getInstance()->get(Ids::RAISIN_CROPS_STAGE_1);
                    $data = Main::getInstance()->blockDataService->get($this->getPosition()->getWorld())->getBlockDataAt(
                        $this->getPosition()->getX(),
                        $this->getPosition()->getY(),
                        $this->getPosition()->getZ()
                    ...
                    break;
                case 1:
                    $block = CustomiesBlockFactory::getInstance()->get(Ids::RAISIN_CROPS_STAGE_2);
                    $data = Main::getInstance()->blockDataService->get($this->getPosition()->getWorld())->getBlockDataAt(
                        $this->getPosition()->getX(),
                        $this->getPosition()->getY(),
                        $this->getPosition()->getZ()
                    ...
                    break;
                case 2:
                    $block = CustomiesBlockFactory::getInstance()->get(Ids::RAISIN_CROPS_STAGE_3);
                    $data = Main::getInstance()->blockDataService->get($this->getPosition()->getWorld())->getBlockDataAt(
                        $this->getPosition()->getX(),
                        $this->getPosition()->getY(),
                        $this->getPosition()->getZ()
    ...
    public function onInteract(Item $item, int $face, Vector3 $clickVector, ?Player $player = null, array &$returnedItems = []) : bool{
        ...
        if ($data instanceof BlockHistoryData) {
            ...
            if($data->getAge() >= 3){
                ...
                $world->dropItem($this->position, CustomiesItemFactory::getInstance()->get(Ids::RAISIN));
                $world->addSound($this->getPosition(), new GlowBerriesPickSound());
                if ($player instanceof CustomPlayer) {
                    $bool = Main::getInstance()->getJobsManager()->addXp($player, JobsManager::FARMER, 5);
                    if ($bool )Main::getInstance()->getJobsManager()->xpNotif($player, $this->getPosition(), 5, JobsManager::FARMER);
                }
            }
    ...
    public function getDrops(Item $item): array
    {
        ...
        if ($this->ageCustom >= 3) {
            return [
                CustomiesItemFactory::getInstance()->get(Ids::RAISIN, 2)
            ];
        } else {
            return [
                CustomiesItemFactory::getInstance()->get(Ids::RAISIN)
            ];
        }
    }
}