...
class BoxCommon extends Entity
{
    ...
    public function __construct(Location $location, ?CompoundTag $nbt = null)
    {
        ...
        $this->setNameTag("§6- §fCommon" . TextFormat::GOLD . "Box §6-\n\n§7§oTape moi");
        $location->pitch = 0.0;
        ...
        $inv->setName("common_box");
        $inv->setContents(array_values(Main::getInstance()->getBoxManager()->getItemsWithBox(BoxManager::COMMON)));
        $this->inv = $inv;
    ...
    {
        $key = CustomiesItemFactory::getInstance()->get(Ids::KEY_COMMON);
        ...
        if ($damager instanceof Player) {
            ...
            $damager->getServer()->dispatchCommand($damager, "open_box common");
            return;
            ...
            Main::getInstance()->getScheduler()->scheduleDelayedTask(new ClosureTask(function () use ($player): void {
                $this->setNameTag("§6- §fBox Common §6-\n\n§7§oTape moi");
    ...
    public function onUpdate(int $currentTick): bool
    {
        ...
        if ($this->isAnimation) {
            if ($this->time === 8) {
                $entitys = $this->getWorld()->getChunkEntities($this->getPosition()->getFloorX() >> 4, $this->getPosition()->getFloorX() >> 4);
