...
class Cow extends Living
{
    ...
    public function getDrops() : array{
        $drops = [
            VanillaItems::RAW_BEEF()->setCount(mt_rand(0, 2)),
    ...
    public function onUpdate(int $currentTick): bool
    {
        ...
        foreach ($entities as $entity) {
            if ($entity instanceof Cow && $entity->getId() !== $this->getId()) {
                if ($entity->stack > $this->stack) {
                    $entity->stack += $this->stack;
                    $this->flagForDespawn();
                    $entity->setNameTag("Vache(s) §6§lx" . $entity->stack);
                } else {
                    $this->stack += $entity->stack;
                    $this->setNameTag("Vache(s) §6§lx" . $this->stack);
