...
class Enderman extends Living
{
    ...
    public function onUpdate(int $currentTick): bool
    {
        ...
        foreach ($entities as $entity) {
            if ($entity instanceof self && $entity->getId() !== $this->getId()) {
                if ($entity->stack > $this->stack) {
                    $entity->stack += $this->stack;
                    $this->flagForDespawn();
                    $entity->setNameTag("Enderman(s) §6§lx" . $entity->stack);
                } else {
                    $this->stack += $entity->stack;
                    $this->setNameTag("Enderman(s) §6§lx" . $this->stack);
    ...
    public function getDrops() : array{
        $drops = [
           CustomiesItemFactory::getInstance()->get(Ids::ENDER_PEARL, 1)
        ];
