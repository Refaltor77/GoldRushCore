...
class Skeleton extends Living
{
    ...
    public function attack(EntityDamageEvent $source): void
    {
        ...
        if ($this->getHealth() - $source->getFinalDamage() <= 2.0) {
            ...
            if ($this->stack <= 0) {
                ...
                $this->setNameTag("Squelettes(s) §6§lx" . $this->stack);
                $source->cancel();
            }
        }
    ...
    public function onUpdate(int $currentTick): bool
    {
        ...
        foreach ($entities as $entity) {
            if ($entity instanceof Skeleton && $entity->getId() !== $this->getId()) {
                if ($entity->stack > $this->stack) {
                    $entity->stack += $this->stack;
                    $this->flagForDespawn();
                    $entity->setNameTag("Squelette(s) §6§lx" . $entity->stack);
                } else {
                    $this->stack += $entity->stack;
                    $this->setNameTag("Squelette(s) §6§lx" . $this->stack);
