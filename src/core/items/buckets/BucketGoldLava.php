...
class BucketGoldLava extends Bucket implements ItemComponents
{
    ...
    {
        $name = 'Seau en or remplie de lave';
        ...
        $this->initComponent('bucket_gold_lava', $inventory);
    ...
    public function onInteractBlock(Player $player, Block $blockReplace, Block $blockClicked, int $face, Vector3 $clickVector, array &$returnedItems): ItemUseResult
    {
        ...
        if ($blockReplace instanceof Lava && $blockReplace->isSource()) {
            ...
            if ($source - 1 > 0) {
                ...
                $this->setLoree(0);
                $player->getInventory()->setItemInHand(CustomiesItemFactory::getInstance()->get(Ids::BUCKET_GOLD_EMPTY)->setNamedTag($this->getNamedTag()));
                $player->getWorld()->setBlock($blockReplace->getPosition(), VanillaBlocks::LAVA());
                $player->getWorld()->addSound($player->getPosition(), new BucketEmptyLavaSound());
            }
        }
