...
class EggChicken extends Item implements ItemComponents
{
    ...
    public function __construct(ItemIdentifier $identifier)
    {
        ...
        $this->initComponent('egg_chicken', $inventory);
        $this->addComponent(new CooldownComponent($this->getName(), 1));

        $this->setLore([
            "§6---",
            "§l§eDescription:§r§f Oeuf de poulet qui permet de faire spawn un poulet\nvous pouvez utiliser cette oeuf\nsur un spawner",
            "§6---",
    ...
    {
        return new Chicken(new Location($pos->getX(), $pos->getY(), $pos->getZ(), $world, $yaw, $pitch));
    }
}