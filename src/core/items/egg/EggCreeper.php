...
class EggCreeper extends Item implements ItemComponents
{
    ...
    public function __construct(ItemIdentifier $identifier)
    {
        ...
        $this->initComponent('egg_creeper', $inventory);
        $this->addComponent(new CooldownComponent($this->getName(), 1));

        $this->setLore([
            "§6---",
            "§l§eDescription:§r§f Oeuf de creeper qui permet de faire spawn un creeper\nvous pouvez utiliser cette oeuf\nsur un spawner",
            "§6---",
    ...
    {
        return new Creeper(new Location($pos->getX(), $pos->getY(), $pos->getZ(), $world, $yaw, $pitch));
    }
}