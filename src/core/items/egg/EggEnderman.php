...
class EggEnderman extends Item implements ItemComponents
{
    ...
    public function __construct(ItemIdentifier $identifier)
    {
        ...
        $this->initComponent('egg_enderman', $inventory);
        $this->addComponent(new CooldownComponent($this->getName(), 1));

        $this->setLore([
            "§6---",
            "§l§eDescription:§r§f Oeuf d'enderman qui permet de faire spawn un enderman\nvous pouvez utiliser cette oeuf\nsur un spawner",
            "§6---",
    ...
    {
        return new Enderman(new Location($pos->getX(), $pos->getY(), $pos->getZ(), $world, $yaw, $pitch));
    }
}