...
class EggMouton extends Item implements ItemComponents
{
    ...
    public function __construct(ItemIdentifier $identifier)
    {
        ...
        $this->initComponent('egg_sheep', $inventory);
        $this->addComponent(new CooldownComponent($this->getName(), 1));

        $this->setLore([
            "§6---",
            "§l§eDescription:§r§f Oeuf de mouton qui permet de faire spawn un mouton\nvous pouvez utiliser cette oeuf\nsur un spawner",
            "§6---",
    ...
    {
        return new Mouton(new Location($pos->getX(), $pos->getY(), $pos->getZ(), $world, $yaw, $pitch));
    }
}