...
class EggZombie extends Item implements ItemComponents
{
    ...
    public function __construct(ItemIdentifier $identifier)
    {
        ...
        $this->initComponent('egg_zombie', $inventory);
        $this->addComponent(new CooldownComponent($this->getName(), 1));

        $this->setLore([
            "§6---",
            "§l§eDescription:§r§f Oeuf de zombie qui permet de faire spawn un zombie\nvous pouvez utiliser cette oeuf\nsur un spawner",
            "§6---",
    ...
    {
        return new Zombie(new Location($pos->getX(), $pos->getY(), $pos->getZ(), $world, $yaw, $pitch));
    }
}