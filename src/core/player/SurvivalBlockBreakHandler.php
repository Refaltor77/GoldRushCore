...
final class SurvivalBlockBreakHandler
{
    ...
    private function calculateBreakProgressPerTick(): float
    {
        ...
        /*
        switch ($this->block->getBreakInfo()->getToolType()) {
            case BlockToolType::PICKAXE:
                if ($this->player->getInventory()->getItemInHand() instanceof CustomPickaxe) {
                    $breakTimePerTick = $this->block->getBreakInfo()->getHardness() * BlockBreakInfo::COMPATIBLE_TOOL_MULTIPLIER * 20;
                    $efficiency = $this->player->getInventory()->getItemInHand()->getMiningEfficiency(true);
                    $breakTimePerTick /= $efficiency;
                }
                break;
            case BlockToolType::AXE:
                if ($this->player->getInventory()->getItemInHand() instanceof CustomAxe) {
                    $breakTimePerTick = $this->block->getBreakInfo()->getHardness() * BlockBreakInfo::COMPATIBLE_TOOL_MULTIPLIER * 20;
                    $efficiency = $this->player->getInventory()->getItemInHand()->getMiningEfficiency(true);
                    $breakTimePerTick /= $efficiency;
                }
            ...
            case BlockToolType::SHOVEL:
                if ($this->player->getInventory()->getItemInHand() instanceof CustomShovel) {
                    $breakTimePerTick = $this->block->getBreakInfo()->getHardness() * BlockBreakInfo::COMPATIBLE_TOOL_MULTIPLIER * 20;
                    $efficiency = $this->player->getInventory()->getItemInHand()->getMiningEfficiency(true);
                    $breakTimePerTick /= $efficiency;
                }
            ...
            case BlockToolType::SWORD:
                if ($this->player->getInventory()->getItemInHand() instanceof CustomSword) {
                    $breakTimePerTick = $this->block->getBreakInfo()->getHardness() * BlockBreakInfo::COMPATIBLE_TOOL_MULTIPLIER * 1.5;
                    $efficiency = $this->player->getInventory()->getItemInHand()->getMiningEfficiency(true);
                    $breakTimePerTick /= $efficiency;
                }
        }
